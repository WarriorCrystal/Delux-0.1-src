...
public class WurstplusHoleUtil
{
    ...
    public static boolean isPlayerInHole(final EntityPlayer entityPlayer) {
        final Vec3d[] hole = { entityPlayer.getPositionVector().add(1.0, 0.0, 0.0), entityPlayer.getPositionVector().add(-1.0, 0.0, 0.0), entityPlayer.getPositionVector().add(0.0, 0.0, 1.0), entityPlayer.getPositionVector().add(0.0, 0.0, -1.0), entityPlayer.getPositionVector().add(0.0, -1.0, 0.0) };
        int holeBlocks = 0;
        final Vec3d[] var3 = hole;
        for (int var4 = hole.length, var5 = 0; var5 < var4; ++var5) {
            final Vec3d vec3d = var3[var5];
            final BlockPos offset = new BlockPos(vec3d.x, vec3d.y, vec3d.z);
            if (WurstplusHoleUtil.mc.world.getBlockState(offset).getBlock() == Blocks.OBSIDIAN || WurstplusHoleUtil.mc.world.getBlockState(offset).getBlock() == Blocks.BEDROCK) {
                ++holeBlocks;
            }
            if (holeBlocks == 5) {
                return true;
    ...
    public static HashMap<BlockOffset, BlockSafety> getUnsafeSides(final BlockPos pos) {
        ...
        BlockSafety temp = isBlockSafe(WurstplusHoleUtil.mc.world.getBlockState(BlockOffset.DOWN.offset(pos)).getBlock());
        if (temp != BlockSafety.UNBREAKABLE) {
            output.put(BlockOffset.DOWN, temp);
        }
        temp = isBlockSafe(WurstplusHoleUtil.mc.world.getBlockState(BlockOffset.NORTH.offset(pos)).getBlock());
        if (temp != BlockSafety.UNBREAKABLE) {
            output.put(BlockOffset.NORTH, temp);
        }
        temp = isBlockSafe(WurstplusHoleUtil.mc.world.getBlockState(BlockOffset.SOUTH.offset(pos)).getBlock());
        if (temp != BlockSafety.UNBREAKABLE) {
            output.put(BlockOffset.SOUTH, temp);
        }
        temp = isBlockSafe(WurstplusHoleUtil.mc.world.getBlockState(BlockOffset.EAST.offset(pos)).getBlock());
        if (temp != BlockSafety.UNBREAKABLE) {
            output.put(BlockOffset.EAST, temp);
        }
        temp = isBlockSafe(WurstplusHoleUtil.mc.world.getBlockState(BlockOffset.WEST.offset(pos)).getBlock());
        if (temp != BlockSafety.UNBREAKABLE) {
